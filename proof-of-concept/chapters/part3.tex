...
\section{Các giới hạn của đồ án}

Trong phạm vi đồ án, hệ thống có các giới hạn sau — đây đồng thời là hướng phát triển cho các phiên bản tương lai:

\begin{itemize}[topsep=0pt, itemsep=4pt, leftmargin=1.5cm]
    \item \textbf{Multiplayer:} Hỗ trợ matchmaking tự động qua hàng đợi, bot fill sau 30s để đủ 4 người. Chưa có chế độ mời bạn bè, phòng riêng hoặc chat trong game.

    \item \textbf{Campaign:} Đã có dữ liệu thẻ cho 3 thành phố: Sài Gòn (Phase 1), Đà Nẵng (Phase 2) và Hà Nội (Phase 3). Tuy nhiên, tính năng Campaign Progression (chơi nối tiếp nhiều phase) và các nhánh bản đồ Đà Lạt, Nha Trang, Huế chưa được triển khai.
    ...
    \item \textbf{Admin panel:} Chưa có giao diện quản trị nội dung. Việc thêm/sửa thẻ địa điểm phải thực hiện thủ công qua file nguồn (\texttt{src/data/cards.phase1.ts}, \texttt{cards.phase2.ts}, \texttt{cards.phase3.ts}).

    \item \textbf{Xuất lịch trình:} Hỗ trợ xuất timeline dạng văn bản và chứng chỉ du lịch (\texttt{src/export/certificate.ts}). Chưa có xuất file PDF hoặc chia sẻ qua link.
\end{itemize}