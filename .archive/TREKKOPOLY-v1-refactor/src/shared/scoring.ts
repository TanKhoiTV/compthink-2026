...
import type { BoardSlots } from "./board.ts";
...
type BuildSimulationReplayStepsParams = {
  ...
  getCurrentDayPlacedCards: (dayIndex?: number) => TravelCard[];
};
...
function getBoardTokenType(card: TravelCard | null) {
  return (card as TravelCard & {
    boardTokenType?: "debt" | "lock";
  } | null)?.boardTokenType ?? null;
}

function isDebtTokenCard(card: TravelCard | null) {
  return getBoardTokenType(card) === "debt";
}

function isLockTokenCard(card: TravelCard | null) {
  return getBoardTokenType(card) === "lock";
}

function getDebtTokenAmount(card: TravelCard | null) {
  return (card as TravelCard & {
    debtAmount?: number;
  } | null)?.debtAmount ?? 0;
}
...
export function buildSimulationReplaySteps({
  ...
  let previousCard: TravelCard | null = null;
...
function getCardLocation(card: TravelCard): { lat: number; lng: number } | null {
  const rawCard = card as TravelCard & {
    lat?: number;
    lng?: number;
...
function getPseudoDistanceKm(
  previousCard: TravelCard,
  currentCard: TravelCard,
  dayIndex: number,
  rowIndex: number
) {
...
function getCardTagKeys(card: TravelCard): string[] {
...
function countCardsByTag(cards: TravelCard[], tag: string): number {
