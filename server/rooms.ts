...
function applyUtilityCardEffect(
	...
	switch (effect.effect_type) {
		...
		case "DOUBLE_VP_NEXT":
			playerWithEffects.doubleVpNext =
				(playerWithEffects.doubleVpNext ?? 0) + effectValue;
			return;
...
export function createRoom(firstPlayerName: string): {
	...
	state.players.p2.isBot = true;
	state.players.p3.isBot = true;
	state.players.p4.isBot = true;
...
export function joinRoom(
	...
	const openPlayerId = PLAYER_IDS.find(
		(playerId) => !state.players[playerId].hasJoined,
	);
...
function getConnectedPlayers(state: RoomState) {
	return PLAYER_IDS.map((playerId) => state.players[playerId]).filter(
