...
import { createRoom, addPlayer, removePlayer, exportSnapshot, type Room } from './game.ts';
import {
    createPlayerSession,
    dispatch,
    sendNotification,
...
async function handleCreateRoom(req: Request): Promise<Response> {
    ...
    if (!Array.isArray(body.cards) || body.cards.length === 0) {
        return errorResponse(400, 'body.cards must be a non-empty array of TravelCard');
    }
...
function handleWebSocket(req: Request): Response {
    ...
    socket.onopen = () => {
        console.log(`[ws] Player ${name} (${playerId}) connected to room ${roomId}.`);
...
console.log(`[server] WebSocket endpoint: ws://${HOST}:${PORT}/ws?roomId=XXXX&playerId=YYY&name=ZZZ`);
