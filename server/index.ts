...
function broadcastMatchmakingStatus() {
	...
	for (const p of matchmakingQueue) {
		io.sockets.sockets
			.get(p.socketId)
			?.emit("matchmaking:status", { count, target: 4 });
	}
}
...
function scheduleMatchmakingTimer() {
	...
	matchmakingTimer = setTimeout(() => {
		...
		const group = matchmakingQueue.splice(
			0,
			Math.min(4, matchmakingQueue.length),
		);
...
function launchMatchmakingGame(
	group: { socketId: string; playerName: string; city: string }[],
) {
	...
	const {
		...
		state,
	} = createRoom(host.playerName, host.city || "SAIGON");
	...
	if (hostSocket) {
		...
		hostSocket.emit("room:joined", {
			roomId,
			playerId: hostPlayerId,
			state: getPlayerViewState(state, hostPlayerId),
		});
	}
	...
	for (let i = 1; i < group.length && i < 4; i++) {
		...
		if (memberSocket && memberPlayerId) {
			...
			memberSocket.emit("room:joined", {
				roomId,
				playerId: memberPlayerId,
				state: getPlayerViewState(state, memberPlayerId),
			});
		}
	}
...
io.on("connection", (socket) => {
	socket.on("room:create", ({ playerName, isTutorial, city }) => {
		const { roomId, playerId, state } = createRoom(
			playerName,
			city || "SAIGON",
		);
	...
	socket.on("matchmaking:find", ({ playerName, city }) => {
		...
		const isAlreadyInQueue = matchmakingQueue.some(
			(p) => p.socketId === socket.id,
		);
		if (!isAlreadyInQueue) {
			matchmakingQueue.push({
				socketId: socket.id,
				playerName: playerName || "Lữ Khách",
				city: city || "SAIGON",
			});
		}
