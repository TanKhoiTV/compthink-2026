...
* **Luồng dữ liệu (Data Flow):** * Client gửi mã phiên bản hiện tại lên Server qua interface **IF-4**.
...
* **Luồng dữ liệu (Data Flow):** * Thao tác chọn thẻ kích hoạt **State Manager (Svelte stores)** cập nhật trạng thái "Hand" của người chơi.
  * Các sự kiện chọn bài được ghi nhận vào **Analytics Queue (IF-3)** để phân tích hành vi người dùng sau này.
...
* **Luồng dữ liệu (Data Flow):** * **Logic validation:** Khi thẻ được đặt vào ô, hệ thống truy xuất tọa độ từ **IndexedDB** để so sánh với thẻ liền trước.
...
* **Luồng dữ liệu (Data Flow):** * **Quest/Event FSM (Finite State Machine)** chạy vòng lặp duyệt thẻ. Với mỗi thẻ, FSM đối chiếu thuộc tính (Tag) để xác định xác suất kích hoạt sự kiện phù hợp.
...
* **Trải nghiệm người dùng (UX) - Input/Output:** * **Input:** Người chơi bấm nút "Hoàn tất" hoặc "Xem bảng xếp hạng" tại màn hình Tổng kết cuối Phase.
...
* **Luồng dữ liệu (Data Flow):** Sau khi kết thúc một hành trình (Run), các dữ liệu về tỷ lệ hoàn thành nhiệm vụ, các điểm tham quan phổ biến và độ khó thực tế sẽ được **Tracking Agent** tự động đóng gói. Dữ liệu này được đẩy lên **Backend API (IF-3)** ngay khi có kết nối mạng mà không làm gián đoạn trò chơi.
