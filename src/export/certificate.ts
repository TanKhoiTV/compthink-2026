...
export function getCertificateHistoryStorageKey() {
  return `${CERTIFICATE_HISTORY_STORAGE_KEY}:${
    onlineClientState.roomId ?? "local"
...
export function formatTravelTimelineAsText() {
  ...
  for (const day of data.timeline) {
    ...
    for (const slot of day.slots) {
      ...
      lines.push(
        `- ${slot.timeLabel}: ${slot.card.name} (${
          slot.card.city || "Không rõ khu vực"
        })`,
      );
      lines.push(
        `  Tag: ${
          slot.card.tagLabel || slot.card.tag
        } • VP: ${slot.card.vp} • Xu: ${slot.card.coin} • Thể lực: ${slot.card.stamina}`,
      );
