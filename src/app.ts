...
function addLocalDebtOrExhaustToken(params: {
	...
	if (getBoardSlots()[nextPosition.rowIndex]?.[nextPosition.colIndex] !== null)
		return;
...
function renderHandCard(
	...
	const isPlanningSelected =
		!state.isDraftPhase && card.id === state.selectedHandCardId;
...
function getDraftHandFlySourceFromElement(
	cardEl: HTMLElement,
...
function renderPlayer(player: Player) {
	...
	const connectionClass =
		onlinePlayer?.isConnected === false ? " side-player--offline" : "";
...
function renderDeckPilePanel() {
	...
	const showDeckHeader =
		showDraftConfirm || showPlanningConfirm || effectTokensHtml.length > 0;
...
function prepareSaigonAlphaCanvas(
...
function isInsideOpaqueSaigonPixel(
	hotspot: SaigonAlphaHotspot,
	bgX: number,
	bgY: number,
) {
...
function applyLobbyBackground() {
	const isLobbyScreen =
		!isOnlineRoomActive() || onlineClientState.roomState?.phase === "lobby";
...
(globalThis as any).toggleInGameBackgroundMusic = (
	window as any
