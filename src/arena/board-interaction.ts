...
export function canPlaceOnBoardCell(rowIndex: number, colIndex: number): boolean {
...
export function placeHandCardOnBoard(cardId: string, rowIndex: number, colIndex: number) {
...
function placeOneBotCard(playerId: PlayerId, sourceCard: TravelCard, seedOffset: number) {
  const board = getPlayerBoards()[playerId];
  const dayIndex = getCurrentDayIndex();
...
function startRealtimeBotPlacement() {
  ...
  botTimerId = window.setInterval(() => {
    ...
    if (countBotCardsInCurrentDay(playerId) < 3) {
      placeOneBotCard(playerId, getPlayerHand()[0] ?? { id: "dummy" } as TravelCard, botIndex);
      rerenderArena();
    }
