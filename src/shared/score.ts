...
import { COST_TO_VND_MULTIPLIER } from "./animations.ts";
import type {
	BoardCell,
	ItineraryEntry,
	PlayerResources,
...
export function calculateScore(
	...
	const placed = board
		.map((cell) => (cell.card_id ? byId.get(cell.card_id) : undefined))
		.filter(Boolean) as TravelCard[];
	...
	const resourceVp =
		Math.floor(resources.xu / 2) + Math.floor(resources.stamina / 2);
...
export function boardToTimeline(
	board: BoardCell[],
	cards: TravelCard[],
): ItineraryEntry[] {
	const byId = new Map(cards.map((card) => [card.card_id, card]));
	return board
		...
		.map((cell) => {
			...
			return {
				...
				estimatedCost: card.cost * COST_TO_VND_MULTIPLIER,
...
function eventRoll(seed: string) {
	return [...seed].reduce(
		(sum, char) => (sum * 31 + char.charCodeAt(0)) % 100,
		7,
	);
}
